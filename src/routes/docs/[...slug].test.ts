/**
 * Test module for Docs page (createDocsPage and getStaticPaths functions).
 *
 * Coverage:
 * - Module exports (createDocsPage, getStaticPaths)
 ...
 * - src/routes/docs/[...slug].ts does not exist yet
 * - createDocsPage and getStaticPaths functions are not implemented
 */
...
describe('Docs Page (createDocsPage)', () => {
  // ==================== Module Exports ====================

  describe('module exports', () => {
    it('should export createDocsPage function', async () => {
      /**
       * Given: [...slug].ts module
       * When: Importing the module
       * Then: Should export createDocsPage function
       *
       * RED PHASE: This test will FAIL - [...slug].ts does not exist
       */
      const module = await import('./[...slug]');
      expect(module).toHaveProperty('createDocsPage');
      expect(typeof module.createDocsPage).toBe('function');
    });
  ...
  // ==================== createDocsPage Structure ====================

  describe('createDocsPage structure', () => {
    it('should return a CompiledNode with kind "element"', async () => {
      /**
       * Given: createDocsPage function
       * When: Called with valid params
       * Then: Should return a CompiledNode with kind "element"
       *
       * RED PHASE: This test will FAIL - [...slug].ts does not exist
       */
      const { createDocsPage } = await import('./[...slug]');
      const result = await createDocsPage({ slug: undefined });
  ...
  describe('content rendering', () => {
    ...
    it('should contain h1 with frontmatter title for index page', async () => {
      /**
       * Given: createDocsPage with undefined slug (index page)
       * When: Inspecting article content
       * Then: Should contain h1 with "Introduction" title from frontmatter
       *
       * RED PHASE: This test will FAIL - [...slug].ts does not exist
       */
      const { createDocsPage } = await import('./[...slug]');
    ...
    it('should contain h1 with frontmatter title for nested slug', async () => {
      /**
       * Given: createDocsPage with ['installation'] slug
       * When: Inspecting article content
       * Then: Should contain h1 with "Installation" title from frontmatter
       *
       * RED PHASE: This test will FAIL - [...slug].ts does not exist
       */
      const { createDocsPage } = await import('./[...slug]');
      const result = await createDocsPage({ slug: ['installation'] });
  ...
  describe('getStaticPaths', () => {
    ...
    it('should contain paths for nested slugs', async () => {
      ...
      // Should have paths like ['installation'], ['first-app'], etc.
    ...
    it('should contain path for installation slug', async () => {
      /**
       * Given: getStaticPaths function
       * When: Inspecting returned paths
       * Then: Should contain path for ['installation']
       *
       * RED PHASE: This test will FAIL - [...slug].ts does not exist
       */
      const { getStaticPaths } = await import('./[...slug]');
      ...
      const installPath = result.find(
        (path) =>
          path.params.slug &&
          path.params.slug.length === 1 &&
          path.params.slug[0] === 'installation'
      );
      expect(installPath).toBeDefined();
    });
  ...
  describe('edge cases', () => {
    it('should handle empty slug (index page)', async () => {
      ...
      await expect(createDocsPage({ slug: undefined })).resolves.toBeDefined();
    });
    ...
    it('should handle empty array slug (index page)', async () => {
      ...
      await expect(createDocsPage({ slug: [] })).resolves.toBeDefined();
    });
    ...
    it('should return 404 structure for non-existent slug', async () => {
      ...
      try {
        const result = await createDocsPage({ slug: ['non-existent-page-xyz'] });
    ...
    it('should be an async function (returns Promise)', async () => {
       ...
       */
      const { createDocsPage } = await import('./[...slug]');
      const result = createDocsPage({ slug: undefined });
    ...
    it('should return consistent structure on multiple calls with same params', async () => {
      ...
      const result1 = await createDocsPage({ slug: undefined });
      const result2 = await createDocsPage({ slug: undefined });
