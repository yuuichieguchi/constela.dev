...
String state can use a **cookie expression** to read the initial value from cookies. This is SSR/SSG-safe and works for theme persistence, user preferences, etc.
...
|----------|------|-------------|
| `expr` | `"cookie"` | Expression type identifier |
| `key` | `string` | Cookie name to read |
| `default` | `string` | Fallback value when cookie is not set |

<Callout type="tip">
Cookie expressions are evaluated during SSR/SSG, so the initial HTML already contains the correct value. This prevents flash of incorrect content (e.g., theme flicker).
</Callout>
...
For more advanced use cases, Constela provides additional expression types. See the [Expressions Reference](/reference/expressions) for complete documentation:
...
- **`call`** - Call methods on arrays, strings, Math, or Date objects
- **`lambda`** - Anonymous functions for array methods (filter, map, find, etc.)
