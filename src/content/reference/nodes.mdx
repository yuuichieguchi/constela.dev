...
<PropsTable items={[
  ...
  { name: "transition", type: "TransitionDirective", required: false, description: "CSS class-based enter/exit transition. See Transitions section below." }
]} />
...
1. **Enter**: When the condition becomes truthy, the `enter` class is applied on the first frame, then `enterActive` on the next frame to trigger the CSS transition
2. **Exit**: When the condition becomes falsy, the `exit` class is applied first, then `exitActive` triggers the transition. The element is removed after `transitionend` fires

#### TransitionDirective

<PropsTable items={[
  { name: "enter", type: "string", required: true, description: "CSS class applied on the initial frame of enter." },
  { name: "enterActive", type: "string", required: true, description: "CSS class applied on the next frame to trigger the enter animation." },
  { name: "exit", type: "string", required: true, description: "CSS class applied on the initial frame of exit." },
  { name: "exitActive", type: "string", required: true, description: "CSS class applied on the next frame to trigger the exit animation." },
  { name: "duration", type: "number", required: false, description: "Fallback duration in milliseconds. Defaults to transitionend event detection." }
]} />
...
<PropsTable items={[
  ...
  { name: "transition", type: "TransitionDirective", required: false, description: "CSS class-based enter/exit transition for list items." }
]} />
...
<PropsTable items={[
  { name: "kind", type: "\"portal\"", required: true, description: "Node type identifier." },
  { name: "target", type: "string", required: true, description: "Target location: \"body\", \"head\", or a CSS selector." },
  { name: "children", type: "ViewNode[]", required: true, description: "Child nodes to render at the target location." }
]} />
...
Renders fallback content while async content is loading. Used with [Streaming SSR](/reference/streaming#suspense-boundaries) for progressive HTML delivery.
...
<PropsTable items={[
  { name: "kind", type: "\"suspense\"", required: true, description: "Node type identifier." },
  { name: "id", type: "string", required: true, description: "Unique identifier for the suspense boundary." },
  { name: "fallback", type: "ViewNode", required: true, description: "Content to show while loading (e.g., skeleton, spinner)." },
  { name: "content", type: "ViewNode", required: true, description: "Async content to load and render." },
  { name: "timeout", type: "number", required: false, description: "Fallback timeout in milliseconds." }
]} />
...
<Callout type="note">
Suspense nodes require [Streaming SSR](/reference/streaming) to be enabled. Without streaming, the server waits for all content before sending the response.
</Callout>
