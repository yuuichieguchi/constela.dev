...
Next.js App Router (13+) integration package with Server-Side Rendering (SSR) support.

```bash
npm install @constela/next
```

<Callout type="note">
**Requirements**: Next.js 13+ (App Router), React 18+, @constela/compiler, @constela/runtime
</Callout>
...
| Export | Description |
|--------|-------------|
| `renderToString` | Renders a CompiledProgram to HTML string for SSR |
| `ConstelaEmbed` | React component for embedding Constela programs |
| `ConstelaEmbedProps` | TypeScript type for component props |

### ConstelaEmbedProps

| Prop | Type | Required | Description |
|------|------|----------|-------------|
| `program` | `CompiledProgram` | Yes | Compiled Constela program |
| `ssrHtml` | `string` | No | Pre-rendered HTML from renderToString() |
| `className` | `string` | No | CSS class for container |
| `id` | `string` | No | ID for container |
...
```typescript
'use client';
import { ConstelaEmbed, type ConstelaEmbedProps } from '@constela/next';

export function DynamicWidget({ program }: Pick<ConstelaEmbedProps, 'program'>) {
  return <ConstelaEmbed program={program} />;
}
```

### How It Works

1. Server-side: `renderToString()` generates HTML
2. Initial render: SSR HTML displays immediately (fast LCP)
3. Client-side: `createApp()` hydrates for interactivity

### Markdown & Code Rendering

Server-side rendering support for `markdown` and `code` nodes:

| Feature | Library | Description |
|---------|---------|-------------|
| Markdown parsing | marked | GFM-compatible Markdown parser |
| Syntax highlighting | shiki | VS Code-quality code highlighting |
