...
Updates a specific field within a nested object or array item. Unlike `set` which replaces the entire target, `setPath` allows surgical updates to deeply nested state.
...
<PropsTable items={[
  ...
  { name: "path", type: "Expression", required: true, description: "Expression that evaluates to an index (for arrays) or key (for objects)." },
  { name: "field", type: "string", required: true, description: "Name of the field to update within the item." },
  { name: "value", type: "Expression", required: true, description: "New value for the field." }
]} />
...
// Toggle completion status of a todo item
{
  ...
  "value": {
    "expr": "un",
    "op": "!",
    "arg": { "expr": "var", "name": "todo", "path": "completed" }
  }
}

// Update username in a nested user object
{
  "do": "setPath",
  "target": "users",
  "path": { "expr": "lit", "value": 0 },
  "field": "profile.name",
