...
<PropsTable items={[
  ...
  { name: "value", type: "Expression", required: false, description: "Value for operations that need one (e.g., push, remove)." }
]} />
...
<PropsTable items={[
  { name: "do", type: "\"clipboard\"", required: true, description: "Step type identifier." },
  { name: "operation", type: "\"write\" | \"read\"", required: true, description: "Clipboard operation to perform." },
  { name: "value", type: "Expression", required: false, description: "Value to write to clipboard (required for 'write' operation)." },
  { name: "result", type: "string", required: false, description: "Variable name to store read result (for 'read' operation)." },
  { name: "onSuccess", type: "ActionStep[]", required: false, description: "Steps to execute on successful clipboard operation." },
  { name: "onError", type: "ActionStep[]", required: false, description: "Steps to execute if clipboard operation fails. Error is available as 'error' variable." }
]} />
...
Clipboard access requires user permission in most browsers. The operation may fail if the user denies permission or if the page is not in a secure context (HTTPS).
</Callout>
...
Executes steps after a specified delay. Useful for notifications, animations, and timed operations.
...
<PropsTable items={[
  { name: "do", type: "\"delay\"", required: true, description: "Step type identifier." },
  { name: "ms", type: "Expression", required: true, description: "Expression that evaluates to delay duration in milliseconds." },
  { name: "then", type: "ActionStep[]", required: true, description: "Steps to execute after the delay." }
]} />
...
<Callout type="tip">
Delay timers are automatically cleaned up when the component unmounts, preventing memory leaks.
</Callout>

## Interval Step

Executes an action repeatedly at a specified interval. Returns a timer ID that can be used with `clearTimer`.
...
<PropsTable items={[
  { name: "do", type: "\"interval\"", required: true, description: "Step type identifier." },
  { name: "ms", type: "Expression", required: true, description: "Expression that evaluates to interval duration in milliseconds." },
  { name: "action", type: "string", required: true, description: "Name of the action to execute on each interval." },
  { name: "result", type: "string", required: false, description: "State field to store the timer ID for later clearing." }
]} />
...
<Callout type="note">
Interval timers are automatically cleaned up when the component unmounts. Store the timer ID in `result` if you need to stop it manually with `clearTimer`.
</Callout>

## ClearTimer Step

Stops a running timer (delay or interval) by its ID.

```json
{
  "do": "clearTimer",
  "target": { "expr": "state", "name": "timerId" }
}
```

<PropsTable items={[
  { name: "do", type: "\"clearTimer\"", required: true, description: "Step type identifier." },
  { name: "target", type: "Expression", required: true, description: "Expression that evaluates to the timer ID to clear." }
]} />
...
### Example: Cancel Pending Delay

```json
{
  "name": "cancelNotification",
  "steps": [
    { "do": "clearTimer", "target": { "expr": "state", "name": "notificationTimerId" } },
    { "do": "set", "target": "notification", "value": { "expr": "lit", "value": "" } }
  ]
}
```
...
Controls focus on form elements. Supports focus, blur, and select operations.

```json
{
  "do": "focus",
  "target": { "expr": "ref", "name": "emailInput" },
  "operation": "focus"
}
```

<PropsTable items={[
  { name: "do", type: "\"focus\"", required: true, description: "Step type identifier." },
  { name: "target", type: "Expression", required: true, description: "Expression that evaluates to a ref name or DOM element." },
  { name: "operation", type: "\"focus\" | \"blur\" | \"select\"", required: true, description: "Focus operation to perform." }
]} />
...
|-----------|-------------|------------|
| `focus` | Sets focus on the element | All focusable elements |
| `blur` | Removes focus from the element | All focusable elements |
| `select` | Selects all text in the element | Input and textarea elements |

### Examples

```json
// Focus an input when form opens
{
  "name": "openForm",
  "steps": [
    { "do": "set", "target": "isFormOpen", "value": { "expr": "lit", "value": true } },
    { "do": "focus", "target": { "expr": "ref", "name": "firstInput" }, "operation": "focus" }
  ]
}
...
<Callout type="tip">
Use `focus` with lifecycle hooks like `onMount` to auto-focus form fields when a modal or form opens.
</Callout>
